...
def func_transform(resultString,hosts):
    ...
    for (k,v) in run_dict.items():
        ...
        Response_string+="<table width='100%' height='15'><tbody><tr><td background='/static/img/B24.gif'></td></tr></tbody></table><br>"
...
def ansible_transform(resultString,hosts):
    ...
    for (hostname, result) in results['contacted'].items():
        if not 'failed' in result:
            Response_string+=u"<font color='#eeeeee'>主机：</font><font color=#ffffff><b>%s</b></font><br>%s<br><font color=#ffffff><b>%s</b></font><br><br>" % (hostname,runstatus_ok,result['stdout'].replace('\n','<br>'))
            Response_string+="<table width='100%' height='15'><tbody><tr><td background='/static/img/B24.gif'></td></tr></tbody></table><br>"
    
    for (hostname, result) in results['contacted'].items():
        if 'failed' in result:
            Response_string+=u"<font color='#eeeeee'>主机: </font><font color=#ffffff><b>%s</b></font><br>%s<br><font color=#ffffff><b>%s</b></font><br>" % (hostname,runstatus_failed, result['msg'])
	    Response_string+="<table width='100%' height='15'><tbody><tr><td background='/static/img/B24.gif'></td></tr></tbody></table><br>"
    
    for (hostname, result) in results['dark'].items():
        Response_string+= u"<font color='#eeeeee'>主机: </font><font color=#ffffff><b>%s</b></font><br>%s<br><font color=#ffffff><b>%s</b></font><br>" % (hostname,runstatus_failed, result)
	Response_string+="<table width='100%' height='15'><tbody><tr><td background='/static/img/B24.gif'></td></tr></tbody></table><br>"
